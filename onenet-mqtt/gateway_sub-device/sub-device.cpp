...
int main(int argc, char *argv[])
{
    if (argc <= 1)
    {
        printf("usage: %s <sub-device-name>\n", argv[0]);
        return -1;
    }
    int sockfd = 0;
    ...
    char tmp_write_buff[128] = {0};
    memcpy(tmp_write_buff, "add_sub_devices:", 16);
    memcpy(tmp_write_buff + 16, argv[1], strlen(argv[1]));
    //处理链接
    int write_ret = write(sockfd, tmp_write_buff, strlen(tmp_write_buff));
    printf("write_ret:%d - [%s]\n", write_ret, tmp_write_buff);
    int epollfd;
