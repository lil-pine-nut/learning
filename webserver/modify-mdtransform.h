...
public:
     ...
     * @return string
     */
    string HandleAngleBrackets(const char *src)
    {
        const char *pos = src;
        string str;
        while (*pos != '\0')
    ...
    // Returns:
    // Qualifier:
    // Parameter: const std::string & fileinfo
    // details :  构造函数对文档树结构处理，生成内容与目录
    //************************************
    MarkdownTransform(const std::string &fileinfo)
    {
        ...
        // 把string转为istringstream
        // string str = HandleAngleBrackets(fileinfo.c_str());
        std::istringstream fin(fileinfo);

        // //从文件流中读取文件
        // std::ifstream fin(fileinfo);
