...
int main(int argc, char **argv)
{
    ...
    int status_code = upload("http://210.76.75.21:8080/iotgz/debug/api/application?version=1&action=CreateDeviceFile", &response);
    if (!status_code)
    {
        return -1;
    }
...
bool upload(string url, string *response)
{
    ...
    if (curl)
    {
        ...
        header_list = curl_slist_append(header_list, "Authorization: version=2020-05-29&res=userid%2F10&et=2524579200&method=md5&sign=wmdn5p1pfRA7%2BficuJspcw%3D%3D");
        ...
        curl_formadd(&post, &last, CURLFORM_PTRNAME, "device_name", CURLFORM_PTRCONTENTS, "subdev_11", CURLFORM_END);
        curl_formadd(&post, &last, CURLFORM_PTRNAME, "product_id", CURLFORM_PTRCONTENTS, "HuU7RhfGd8", CURLFORM_END);
        ...
        curl_formadd(&post, &last, CURLFORM_PTRNAME, "filename", CURLFORM_PTRCONTENTS, "test-curl_version2.txt", CURLFORM_END);
        ...
        curl_formadd(&post, &last,
                     ...
                     CURLFORM_BUFFER, "test-curl_version2.txt",
